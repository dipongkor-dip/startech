  ...
  return (
    <div className="w-full max-w-md bg-card rounded-xl shadow-lg p-8">
      ...
      <div className="flex gap-2 mb-6">
        <Button type="button" onClick={() => setMode("login")} className={`flex-1 py-2 rounded-lg font-medium ${mode === "login" ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-600"}`}>
          Login
        </Button>
        <Button type="button" onClick={() => setMode("register")} className={`flex-1 py-2 rounded-lg font-medium ${mode === "register" ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-600"}`}>
          Register
        </Button>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {mode === "register" && (
          <div>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name" className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-lg outline-none" />
          </div>
        )}
        <div>
      ...
      <div className="space-y-3">
        <Link href={`${API_URL}/auth/google`} className="flex items-center justify-center gap-2 w-full py-3 border border-gray-300 dark:border-gray-700 rounded-lg font-medium">
        ...
        <Link href={`${API_URL}/auth/facebook`} className="flex items-center justify-center gap-2 w-full py-3 border border-gray-300 dark:border-gray-700 rounded-lg font-medium">
