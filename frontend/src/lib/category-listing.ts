...
function buildPills(cat: NavCategory) {
  return cat.items
    .filter((i) => !i.label.toLowerCase().startsWith('show all'))
...
export function mapPhonesToMockProducts(phones: PhoneLike[]): MockProduct[] {
  return phones.map((p) => {
    const discountPrice =
      typeof p.discountPrice === 'number' && p.discountPrice > 0 ? p.discountPrice : undefined;
